...
* **检查约束： 关键字是  CHECK** 
  ...
  > 注意：MySQL不支持检查约束。在大于等于版本8.0.16开始完整支持。
  >
...
* 语法
  ...
  * 删除约束
    ...
    ```sql
    ALTER TABLE 表名 MODIFY 字段名 数据类型;            -- 什么都不加就表示删除了所有约束
    ```
...
* 语法
  ...
  * 添加约束
    ...
    CREATE TABLE 表名(
       列名 数据类型,
       …
       [CONSTRAINT] [约束名称] UNIQUE(列名)           -- CONSTRAINT: 声明约束名称
    ); 
    ```
  ...
  * 删除约束
    ...
    ```sql
    ALTER TABLE 表名 DROP INDEX 字段名;                              -- 用于删除非主键索引，所有约束都是一种索引
    ```
...
增加auto_increment之后的主键也可以插入null了，或者不写，sql会自动自增。
